...
.config([
    ...
    function ($routeProvider, $httpProvider) {
    	...
    	$routeProvider.when('/agilize/payment/create/:amount/:nmAplic', {
            templateUrl: 'views/initTran.html',
    ...
    '$rootScope', '$scope', '$location', 'APP_CONFIG', 'Payment', '$modal', '$msgbox', 'amount', 'nmAplication', '$store',
    function ($rootScope, $scope, $location, APP_CONFIG, Payment, $modal, $msgbox, amount, nmAplication, $store) {
    	
    	$scope.payment = new Payment();
    	$scope.payment.sale = {payment: {amount: amount, returnUrl: APP_CONFIG.host,
    	...
    	//Payment.application(nmAplication, function(){}).then(function (result) {
    		//var applic = $scope.onSuccess(result);
		$scope.payment.application = {name: nmAplication};
		//});
		//$scope.payment.token = $rootScope.login.token;
