angular.module('restResource', ['msgbox', 'services.localizedMessages']).factory('restResource', ['APP_CONFIG','$http', '$q', '$msgbox', 'localizedMessages', '$rootScope',
  ...
  function restResourceFactory(collectionName,substantive) {
    ...
    Resource.ufAll = function (successcb, errorcb) {
  		$http.defaults.headers.common[APP_CONFIG.keyPathUrl] = 'uf/all';
  		$http.defaults.headers.common[APP_CONFIG.keyApplication] = APP_CONFIG.nmApplicationElasticCode;
		var httpPromise = $http.get(url);
		$http.defaults.headers.common[APP_CONFIG.keyApplication] = APP_CONFIG.nmApplication;
		return thenFactoryMethod(httpPromise, successcb, errorcb, false);
    };
